...
fn main() {
  ...
  // SAFETY:
  // Even though `noise` has sufficient capacity and the node seems correctly
  // configured (with valid node names and parameter types), undefined behavior
  // may still occur during noise generation. Potential issues include:
  // - Incorrect or incomplete parameter setup (e.g., missing source nodes or
  //   invalid values).
  // - Internal errors or limitations in the FastNoise2 library that may not be
  //   evident from Rust's type safety or runtime checks.
