// This example illustrates the use of different types of writing with
// generators to build safe trees.
...
fn create_node() -> GeneratorWrapper<SafeNode> {
  ...
  // Please note that using "0.5" (a float) instead of "Constant { value : 0.5 }"
  // (a node) is faster because the right-hand member of the "Add" node is a
  // hybrid member (either a node or a float) that FastNoise2 handles
  // differently. It is also easier to write "0.5".
  let _n = Add {
    lhs: Fade {
  ...
  // You can, and I would recommend this, use functions to instance different
  // types of nodes wrapped in a Generator, necessary for operators such as + - *
  // /
  ...
  // You can also mix the two writings. Note the use of the GeneratorWrapper type
  // to enable use of the operator
  ...
  // Qualifying the "fade" method can also lead to better syntax, although this is
  // subjective.
  let _n = GeneratorWrapper::fade(sinewave(0.1), sinewave(-0.2), simplex()) + 0.5;

  // simplex() takes two unnecessary parentheses, so you can create the Simplex
  // structure directly, since GeneratorWrapper is not needed here.
  let _n = GeneratorWrapper::fade(sinewave(0.1), sinewave(-0.2), Simplex) + 0.5;

  // In the end, this is the most idiomatic writing, and it's easier to import
  // functions by using "use fastnoise2::generator::prelude::*;"
