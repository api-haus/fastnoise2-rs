...
impl Generator for SineWave {
  ...
  fn build(&self) -> GeneratorWrapper<SafeNode> {
    ...
    // FastNoise2 renamed Scale to FeatureScale
    node.set("FeatureScale", self.scale).unwrap();
    SafeNode(node.into()).into()
  }
}
...
impl Generator for PositionOutput {
  #[cfg_attr(feature = "trace", tracing::instrument(level = "trace"))]
  fn build(&self) -> GeneratorWrapper<SafeNode> {
    // FastNoise2 replaced PositionOutput with Gradient
    let mut gradient = Node::from_name("Gradient").unwrap();
    gradient.set("MultiplierX", self.x_multiplier).unwrap();
    gradient.set("MultiplierY", self.y_multiplier).unwrap();
    gradient.set("MultiplierZ", self.z_multiplier).unwrap();
    gradient.set("MultiplierW", self.w_multiplier).unwrap();
    gradient.set("OffsetX", self.x_offset).unwrap();
    gradient.set("OffsetY", self.y_offset).unwrap();
    gradient.set("OffsetZ", self.z_offset).unwrap();
    gradient.set("OffsetW", self.w_offset).unwrap();
    SafeNode(gradient.into()).into()
  }
}
...
impl Generator for DistanceToPoint {
  ...
  fn build(&self) -> GeneratorWrapper<SafeNode> {
    ...
    node
      .set("DistanceFunction", &*self.distance_function.to_string())
      .unwrap();
