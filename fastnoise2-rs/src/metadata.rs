...
pub enum MemberType {
  ...
  NodeLookup,
  /// A member that can be either a floating-point value ([`f32`]) or a [`Node`]
  /// reference.
  Hybrid,
}
  ...
  for id in 0..metadata_count {
    ...
    for variable_idx in 0..variable_count {
      ...
      let name = format_dimension_member(
        &format_lookup(
          &unsafe { CStr::from_ptr(fnGetMetadataVariableName(id, variable_idx)) }.to_string_lossy(),
        ),
        dimension_idx,
      );
      ...
      if let MemberType::Enum = member_type {
        ...
        for enum_idx in 0..enum_count {
          let enum_name = format_lookup(
            &unsafe { CStr::from_ptr(fnGetMetadataEnumName(id, variable_idx, enum_idx)) }
              .to_string_lossy(),
          );
    ...
    for hybrid_idx in 0..hybrid_count {
      ...
      let name = format_dimension_member(
        &format_lookup(
          &unsafe { CStr::from_ptr(fnGetMetadataHybridName(id, hybrid_idx)) }.to_string_lossy(),
        ),
        dimension_idx,
      );
      members.insert(
