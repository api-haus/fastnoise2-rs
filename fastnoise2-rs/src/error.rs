...
///
/// This enum covers various failure scenarios including metadata issues, value
/// setting problems, and node creation errors.
#[derive(Error, Debug)]
pub enum FastNoiseError {
  /// Indicates that the provided metadata name was not found.
  ///
  /// FastNoise2 uses metadata to manage node names and parameters. This error
  /// occurs if the given metadata name is not recognized.
  #[error(
  ...
  /// Indicates a failure to create a [`CString`][`std::ffi::CString`] from the
  /// provided encoded node tree string.
  ...
  ///
  /// This error occurs if the member name specified is not available for the
  /// node.
  #[error(
  ...
  ///
  /// This error occurs when there is a mismatch between the expected member
  /// type and the provided value type.
  ...
  ///
  /// This error occurs if the provided enum value does not match any of the
