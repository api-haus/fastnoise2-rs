...
/// Unlike [`Node`], this structure is safe to use because it is built from
/// typed nodes that implement the [`Generator`][`crate::generator::Generator`] trait, or built by an encoded node tree produced by [NoiseTool](https://github.com/Auburn/FastNoise2?tab=readme-ov-file#noise-tool).
///
...
impl SafeNode {
  ...
  pub fn gen_uniform_grid_2d(
    ...
    unsafe {
      self.0.gen_uniform_grid_2d_unchecked(
        noise_out, x_offset, y_offset, x_count, y_count, x_step_size, y_step_size, seed,
      )
    }
  }
  ...
  pub fn gen_uniform_grid_3d(
    ...
    unsafe {
      self.0.gen_uniform_grid_3d_unchecked(
        noise_out, x_offset, y_offset, z_offset, x_count, y_count, z_count, x_step_size,
        y_step_size, z_step_size, seed,
      )
    }
  }
  ...
  pub fn gen_uniform_grid_4d(
    ...
    unsafe {
      self.0.gen_uniform_grid_4d_unchecked(
        noise_out, x_offset, y_offset, z_offset, w_offset, x_count, y_count, z_count, w_count,
        x_step_size, y_step_size, z_step_size, w_step_size, seed,
      )
    }
  }

  /// # Panics
  /// Panics if `noise_out`, `x_pos_array`, and `y_pos_array` do not have the
  /// same length.
  ...
  /// # Panics
  /// Panics if `noise_out`, `x_pos_array`, `y_pos_array`, and `z_pos_array` do
  ...
  /// # Panics
  /// Panics if `noise_out`, `x_pos_array`, `y_pos_array`, `z_pos_array` and
  /// `w_pos_array` do not have the same length.
  ...
  pub fn gen_tileable_2d(
    ...
    unsafe {
      self
        .0
        .gen_tileable_2d_unchecked(noise_out, x_size, y_size, x_step_size, y_step_size, seed)
    }
  }
